        ...
        <div class="col-lg-12">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.compte.title_singular') }}
                </div>
                <div class="panel-body">

                    <form action="{{ route("admin.comptes.update", [$compte->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group {{ $errors->has('type_compte') ? 'has-error' : '' }}">
                            ...
                            <select id="type_compte" name="type_compte" class="form-control" required>
                                <option value="" disabled {{ old('type_compte', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                @foreach(App\Compte::TYPE_COMPTE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('type_compte', $compte->type_compte) === (string)$key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
